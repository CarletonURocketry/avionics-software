        ...
        union {
            ...
            union {
                struct {
                    /** Pin number */
                    uint8_t pin:5;
                    /** Port, 0 for A, 1 for B, 2 for C, 3 for D */
                    uint8_t port:3;
                };
...
#define PIN_NONE ((union gpio_pin_t){ .type = GPIO_INVALID_PIN })
...
/**
 *  Check if a pin is invalid (e.g. PIN_NONE).
 *
 *  @param pin The pin to check
 *
 *  @return 1 if the pin is invalid, 0 otherwise
 */
static inline uint8_t gpio_pin_is_invalid(union gpio_pin_t pin)
{
    return pin.type == GPIO_INVALID_PIN;
}
