              ...
              const struct usb_configuration_descriptor *config_desc)
{
    ...
    /* Enable the AHB and APB clocks for the USB */
    enable_bus_clock(PERPH_BUS_CLK_USB_AHB);
    enable_bus_clock(PERPH_BUS_CLK_USB_APB);
    
    /* Select the clock for the USB */
    set_perph_generic_clock(PERPH_GCLK_USB, clock_mask);
...
// MARK: ISR
static inline uint8_t handle_device_interrupts(void)
{
...
static inline void handle_ep_trcpt0_interrupt(uint8_t i)
{
    if (USB->DEVICE.DeviceEndpoint[i].EPINTFLAG.bit.TRCPT0) {
        /* Out complete */
        if (usb_out_callbacks_g[i] != NULL) {
            usb_out_callbacks_g[i](usb_ep_descriptors_g[i].DeviceDescBank[0].PCKSIZE.bit.BYTE_COUNT);
        }
        // Clear interrupt flag
...
static inline void handle_ep_trcpt1_interrupt(uint8_t i)
{
...
void USB_Handler (void)
{
    ...
    while (USB->DEVICE.EPINTSMRY.reg) {
        ...
        handle_ep_trcpt0_interrupt(i);
        handle_ep_trcpt1_interrupt(i);
...
{
    uint8_t ints = USB->DEVICE.EPINTSMRY.reg;
    while (ints != 0) {
        uint8_t i = __builtin_ctz(USB->DEVICE.EPINTSMRY.reg);
        handle_ep_trcpt0_interrupt(i);
        ints &= ~(1 << i);
